...
public class MinWinStr {
	...
	public static String minWindow(String s, String t) {
		...
		// We will use two variables 'have' & 'need' to keep a track whether the
		// characters
		...
		for (int windowEnd = 0; windowEnd < s.length(); windowEnd++) {
			...
			/**
			 * check if sMap has required character same as in tMap
			 */
			if (tMap.containsKey(ch) && sMap.get(ch) <= (tMap.get(ch))) {
				have++;
			}
			// if have is equals to the need, means we got a substring in s having all the
			// character of t
			while (have == need) {
