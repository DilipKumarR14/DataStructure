...
class Solution {
	...
	public boolean isValidSudoku(char[][] board) {
		...
		for (int row = 0; row < 9; row++) {
			for (int column = 0; column < 9; column++) {
				// If the position is a number (!= '.'), and it is a not a valid placement, then
				// return false.
	...
	private boolean isNumberInColumn(char[][] board, char number, int row, int column) {
		for (int i = 0; i < 9; i++) {
			...
			// Otherwise, check if any identical number in the column and return true if
			// found.
	...
	// Additional wrapper method for all three checks to make the code more
	// readable.
	private boolean isValidPlacement(char[][] board, char number, int row, int column) {
		// If the number is not found in the row, column and the box, then it is a valid
